...
echo "downsize initial pictures max widths to %target_width%px..."
for %%a in ("_inputs\*.*") do (
   call tools\scale.bat -source "%%~fa" -target "_outputs-rescale\%%~na.jpg" -max-width %target_width% -keep-ratio yes -force yes
)
pause
